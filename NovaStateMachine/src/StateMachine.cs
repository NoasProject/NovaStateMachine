...
{
    public abstract class StateMachine : State, IState
    {
        // -------------------------------------------------------------
        // プライベート変数
        ...
        private TransitionIdentity _nextTransition;
        ...
        void IState.Enter()
        {
            ...
            if (!this._currentIdentity.IsValid && this._startingStateIdentity.IsValid)
            {
                this._currentIdentity = this._startingStateIdentity;
            }
            ...
            // 子の処理を後に行う
            this._currentIdentity.State?.Enter();
        }

        void IState.Update(long elapsedMs)
        {
            // 次の遷移が設定されている場合は、遷移処理を行う
            if (this._nextTransition != null)
            {
                this.TransitionProcess(this._nextTransition);
                this._nextTransition = null;
                return;
            }

            // 遷移が終わった後にUpdate処理を行う
            this._currentIdentity.State?.Update(elapsedMs);
        ...
        internal bool TransitionInternal(string transitionName, bool isForce = false)
        {
            ...
            if (!this._currentIdentity.IsValid)
            {
                return false;
            }

            // すでに遷移が設定されている場合は処理をしない
            if (this._nextTransition != null && !isForce)
            {
                return false;
            }
